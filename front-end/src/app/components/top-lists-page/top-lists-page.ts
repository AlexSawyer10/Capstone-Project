...
@Component({
  selector: 'app-top-lists-page',
  imports: [CommonModule],
  templateUrl: './top-lists-page.html',
  styleUrl: './top-lists-page.css',
})
export class TopListsPage {
  ...
  ngOnInit(): void {
    ...
    this.listService.getTopLists().subscribe({
      next: (response) => {
  ...
  private applyVoteToList(listId: number, response: any): void {
    this.lists.update(lists => {
      ...
      return updated.sort((a, b) => (b.listLikes ?? 0) - (a.listLikes ?? 0));
    });
